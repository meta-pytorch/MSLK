...
def _kernel_quantize_fp8_tensor(
    ...
    if old_count == num_sms - 1:
        ...
        scale = MAX_FP8 / global_max_float
        tl.store(scale_out_ptr, 1.0 / scale)
...
def triton_quantize_fp8_tensor(
    ...
    Args:
        a (Tensor): Contiguous input tensor (flattened view).
...
def quantize_fp8_tensor_meta(
    ...
    """Shape function for torch compile."""
    dtype = get_fp8_constants()[0]
    fake_out = torch.empty(a.shape, device=a.device, dtype=dtype)
