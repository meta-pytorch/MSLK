...
def get_clock_rate_in_khz():
    try:
        ...
        # nvidia-smi is unavailable (e.g. on AMD/ROCm GPUs). Triton's device
        # properties expose the SM clock in kHz on both the CUDA and ROCm
        # backends, so use that instead of pynvml, which is Nvidia-only and is
        # not installed in the ROCm CI image.
