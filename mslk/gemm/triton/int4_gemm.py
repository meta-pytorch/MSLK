...
def _bf16i4_rowwise_kernel(
    ...
    """
    NUM_XCDS: tl.constexpr = 8  # pyre-ignore: Triton constexpr literal assignment
    pid = tl.program_id(0)
    ...
    if EVEN_MN:
        ...
        # Wrap only the load addresses (%M / %N) to keep them in bounds; the raw
        # unwrapped indices are retained for masking so out-of-range tail rows/cols
        # are excluded on store instead of wrapping onto valid earlier rows/cols.
        offs_m = offs_m_raw % M
        offs_n = offs_n_raw % N
    ...
    for k2_idx in tl.range(0, num_iters):
        ...
        if FUSE_DOT:
            ...
            w_fused = tl.cat(
                w_lo_dq.to(tl.bfloat16),
                w_hi_dq.to(tl.bfloat16),
                dim=1,  # pyre-ignore[28]
            )
    ...
    if EVEN_MN:
        ...
        tl.store(
            y_ptrs, acc, mask=(offs_m_raw[:, None] < M) & (offs_n_raw[None, :] < N)
        )
