        ...
        for m in self.ms:
            for n, k in nks:
                ...
                for block_dim_x, block_dim_y in block_dims:
                    ...
                    # e.g. https://www.internalfb.com/code/fbsource/[97d1b331c42f]/fbcode/mslk/bench/quantize/quantize_ops.py?lines=426
                    if quantize_w and quantize_x:
                ...
                if best_elapsed_time is None:
                    ...
                    return
                if fn == torch.ops.mslk.bf16_fast_gemv:
                    bw = (
                        (m * k * 2 + n * k * 2 + m * n * 2)
                        / (best_elapsed_time / 1000)
                        / (1024**3)
                    )
                elif fn == torch.ops.mslk.bf16fp8bf16_fast_gemv:
                    bw = (
                        (m * k * 2 + n * k + m * n * 2)
                        / (best_elapsed_time / 1000)
                        / (1024**3)
                    )
                else:  # Assuming fn is torch.ops.mslk.fp8fp8bf16_fast_gemv
                    bw = (
                        (m * k + n * k + m * n * 2)
                        / (best_elapsed_time / 1000)
                        / (1024**3)
...
sweep_instance.sweep_heuristics(fn=torch.ops.mslk.bf16_fast_gemv)
sweep_instance.sweep_heuristics(
    fn=torch.ops.mslk.bf16fp8bf16_fast_gemv, quantize_w=True
)
sweep_instance.sweep_heuristics(
    fn=torch.ops.mslk.fp8fp8bf16_fast_gemv, quantize_w=True, quantize_x=True
)

# 17bx128e dense l4 model shapes
sweep_instance.sweep_heuristics(
    fn=torch.ops.mslk.fp8fp8bf16_fast_gemv,
    quantize_w=True,
    quantize_x=True,
    llama_4=True,
)
