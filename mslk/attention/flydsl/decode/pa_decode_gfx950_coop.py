...
def compile_pa_decode_gfx950_coop(
    *,
    ...
    assert arch.startswith("gfx950"), (
        f"pa_decode_gfx950_coop requires gfx950 (ds_read_tr16_b64), got {arch}"
    )
    ...
    _mfma_pv = (
        ...
        if kv_dtype_str == "bf16"
        else rocdl.mfma_f32_32x32x16_f16
    )
    ...
    def pa_decode_gfx950_coop_kernel(
        ...
        p_lds = SmemPtr(
            smem, (_K_LDS_F16 + _V_LDS_F16) * 2, T.f32, shape=(_P_LDS_F32,)
        ).get()
        ...
        for _tile_i, state in range(
            fx.Index(t_start), fx.Index(t_end), arith.index(TILE_N), init=_init_state
        ):
            ...
            for d in range_constexpr(_NUM_DMA_KV):
                row_in_tile = tid // fx.Index(_LANES_PER_ROW) + fx.Index(
                    d * _ROWS_PER_ROUND
                )
                col_f16 = (tid % fx.Index(_LANES_PER_ROW)) * fx.Index(8)
                global_row = tile_start + fx.Int32(row_in_tile)
                k_voffset = (
                    kv_base + global_row * stride_km + fx.Int32(col_f16)
                ) * fx.Int32(2)
                k_lds_rb = (
                    k_lds_base_bytes + _wave_dma_offset + fx.Index(d * _DMA_BATCH)
                )
                rocdl.raw_ptr_buffer_load_lds(
                    k_rsrc,
                    buffer_ops.create_llvm_ptr(
                        rocdl.readfirstlane(fx.Int64.ir_type, fx.Int64(k_lds_rb)),
                        address_space=3,
                    ),
                    _dma_size,
                    k_voffset,
            ...
            for d in range_constexpr(_NUM_DMA_KV):
                ...
                v_lds_rb = (
                    v_lds_base_bytes + _wave_dma_offset + fx.Index(d * _DMA_BATCH)
                )
                rocdl.raw_ptr_buffer_load_lds(
                    v_rsrc,
                    buffer_ops.create_llvm_ptr(
                        rocdl.readfirstlane(fx.Int64.ir_type, fx.Int64(v_lds_rb)),
                        address_space=3,
                    ),
                    _dma_size,
                    v_voffset,
            ...
            for td in range_constexpr(TLOOP):
                ...
                for g in range_constexpr(_QK_GROUPS):
                    ...
                    k_byte = k_lds_base_bytes + (
                        fx.Index(k_tok) * fx.Index(_HEAD) + fx.Index(k_col)
                    ) * fx.Index(2)
                    k_ptr = buffer_ops.create_llvm_ptr(
                        fx.Int64(k_byte), address_space=3
                    )
                    k_v8s.append(
                        _llvm.LoadOp(
                            T.vec(8, _FX_KV.ir_type), k_ptr, alignment=16
                        ).result
                    )
                qk_acc = zero_v4
                for g in range_constexpr(_QK_GROUPS):
                    qk_acc = _mfma_qk(
                        T.vec(4, T.f32), [q_frags[g], k_v8s[g], qk_acc, 0, 0, 0]
                    )
            ...
            for td in range_constexpr(TLOOP):
                ...
                p_c = _exp2_fast(
                    fx.Float32(
                        arith.mulf(
                            arith.subf(
                                arith.unwrap(qk_scalars[td]), arith.unwrap(safe_max)
                            ),
                            arith.constant(LOG2E, type=T.f32),
                        )
                    )
                ...
                vector.store(
                    fx.Vector.from_elements([arith.unwrap(p_c)], dtype=fx.Float32),
                    p_lds,
                    [p_slot],
                )
            ...
            for dc in range_constexpr(_D_CHUNKS):
                ...
                for e in range_constexpr(16):
                    c_acc = vector.insert(
                        arith.mulf(pv_scalars[dc * 16 + e], rescale_raw),
                        c_acc,
                        static_position=[e],
                        dynamic_position=[],
                    )
                ...
                for pks in range_constexpr(PV_K_STEPS):
                    ...
                    v_lds_lo_f16 = (
                        v_lds_base_bytes // fx.Index(2)
                        + k_row_base * fx.Index(_V_STRIDE)
                        + d_col_base
                    )
                    v_lds_lo_byte = v_lds_lo_f16 * fx.Index(2)
                    v_lds_hi_byte = v_lds_lo_byte + fx.Index(
                        8 * _V_STRIDE * 2
                    )  # +8 toks

                    lo_ptr = buffer_ops.create_llvm_ptr(
                        fx.Int64(v_lds_lo_byte), address_space=3
                    )
                    hi_ptr = buffer_ops.create_llvm_ptr(
                        fx.Int64(v_lds_hi_byte), address_space=3
                    )
                    v_lo_v4 = rocdl.ds_read_tr16_b64(
                        v4f16_type, lo_ptr
                    ).result  # k=0..3
                    v_hi_v4 = rocdl.ds_read_tr16_b64(
                        v4f16_type, hi_ptr
                    ).result  # k=4..7
                    ...
                    for j in range_constexpr(8):
                        tok_j = (
                            fx.Int32(pks * PV_K_STEP)
                            + lane_div_32 * fx.Int32(4)
                            + fx.Int32(j % 4)
                            + fx.Int32((j // 4) * 8)
                        )
                        p_slot = warp_id * fx.Int32(TILE_N) + tok_j
                        pf = fx.Vector.load(T.vec(1, T.f32), p_lds, [fx.Index(p_slot)])[
                            0
                        ]
                        p_f16 = arith.truncf(
                            _FX_KV.ir_type, arith.unwrap(fx.Float32(pf))
                        )
                    ...
                    c_acc = _mfma_pv(T.vec(16, T.f32), [v_frag, p_frag, c_acc, 0, 0, 0])
            ...
            for dc in range_constexpr(_D_CHUNKS):
                for e in range_constexpr(16):
                    ...
                    if const_expr(_SPLIT):
                        ...
                        out_val = _FX_OUT(
                            arith.unwrap(fx.Float32(arith.mulf(pv_val, inv_raw)))
                        )
...
def _make_gfx950_coop_jit_launcher(head_size, kv_dtype_str, out_dtype_str, split_k):
    kernel, _alloc = compile_pa_decode_gfx950_coop(
        head_size=head_size,
...
def pa_decode_gfx950_coop_launch(
    ...
    if split_k == 0:
        split_k = auto_split_k_coop(B, G, H_q, KV_MAX)
