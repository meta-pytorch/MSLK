...
def get_warp_size(arch: Optional[str] = None) -> int:
    """Wavefront/warp size: wave64 for CDNA (gfx9xx), wave32 for RDNA."""
    if arch is None:
        arch = get_rocm_arch()
    return 32 if is_rdna_arch(arch) else 64


def smem_bytes(arch: Optional[str] = None) -> int:
...
def exp_f32(value):  # pyre-ignore[2,3]
    """Scalar `e^value` via mlir math.exp (lowers to __ocml_exp_f32 on CDNA).
...
def _dpp_xor_i32_raw(src_i32, offset: int):  # pyre-ignore[2,3]
    """Butterfly-XOR within a 16-lane row via llvm.amdgcn.update.dpp.i32.

    Valid only for offsets 1,2,4,8 (within-row DPP on CDNA wave64); for offsets
    16,32 use shuffle_xor/ds_swizzle. DPP control values (AMD ISA / aiter ref):
      offset=8 → two-pass mask 0xC then 0x3;  offset=4 → 0xA then 0x5
      offset=2 → dpp_ctrl=78;  offset=1 → dpp_ctrl=177
    """
