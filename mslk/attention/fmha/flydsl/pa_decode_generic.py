    ...
    def pa_decode_generic_kernel(
        ...
        for _tile_i, state in range(_t_s, _t_e, arith.index(TILE_N), init=_init_state):
            ...
            for td in range_constexpr(TLOOP):
                ...
                k_frags_td = []
                for g in range_constexpr(_QK_GROUPS):
                    k_off = kv_base + tok_td * stride_km + fx.Int32(g * MFMA_K) + k_grp * fx.Int32(8)
                    k_frags_td.append(
            ...
            for td in range_constexpr(TLOOP):
                ...
                for g in range_constexpr(_QK_GROUPS):
                    qk_acc = _mfma(
                        T.vec(4, T.f32), [q_frags[g], k_frags_all[td][g], qk_acc, 0, 0, 0])
            ...
            for td in range_constexpr(TLOOP):
                tok_td  = tile_start + fx.Int32(td * MFMA_N) + tok_qk
                qk_raw  = vector.extract(qk_vecs[td], static_position=[0], dynamic_position=[])
            ...
            safe_max = fx.Float32(arith.select(
                arith.unwrap(new_max) > c_neginf,
                arith.unwrap(new_max), c_zero))
            p_vals = []
            ...
            for td in range_constexpr(TLOOP):
                ...
                p_c = _exp2_fast(fx.Float32(arith.mulf(
                    arith.subf(arith.unwrap(qk_vals[td]), arith.unwrap(safe_max)),
                    arith.constant(LOG2E, type=T.f32))))
            ...
            for sh in (8, 4, 2, 1):
                intra_sum = fx.Float32(arith.addf(
            ...
            for td in range_constexpr(TLOOP):
                ...
                vector.store(fx.Vector.from_elements([arith.unwrap(p_vals[td])], dtype=fx.Float32),
                             p_lds, [p_slot])
            gpu.barrier()
            ...
            p_half_base = [warp_id * fx.Int32(TILE_N), warp_id * fx.Int32(TILE_N) + fx.Int32(TILE_N // 2)]
            p_frags = []
            for half in range_constexpr(2):
                p_frag = zero_v8h
                pbase = p_half_base[half] + k_grp * fx.Int32(8)
                for j in range_constexpr(8):
                    pf_j = fx.Vector.load(T.vec(1, T.f32), p_lds,
                                           [fx.Index(pbase + fx.Int32(j))])[0]
            ...
            for half in range_constexpr(2):
                ...
                for g in range_constexpr(_PV_GROUPS):
                    gvals = []
                    for j in range_constexpr(8):
                        tok_j = tile_start + fx.Int32(half * (TILE_N // 2)) + k_grp * fx.Int32(8) + fx.Int32(j)
                        d_out = fx.Int32(g * MFMA_N) + tok_qk
                        v_off = kv_base + tok_j * stride_km + d_out
                        v_val = buffer_ops.buffer_load(v_rsrc, v_off, vec_width=1, dtype=_FX_KV)
            ...
            for g in range_constexpr(_PV_GROUPS):
                ...
                for e in range_constexpr(4):
                    c_acc = vector.insert(arith.mulf(pv_scalars[g * 4 + e], rescale_raw),
                                          c_acc, static_position=[e], dynamic_position=[])

                for half in range_constexpr(2):
                    v_frag = zero_v8h
                    for j in range_constexpr(8):
                        v_frag = vector.insert(v_pf[half][g][j], v_frag,
                                               static_position=[j], dynamic_position=[])
                    c_acc = _mfma(
        ...
        safe_sum = fx.Float32(arith.select(arith.unwrap(final_sum) > c_zero,
                                           arith.unwrap(final_sum), c_one))
        ...
        if tok_qk == fx.Int32(0):
            if hq_abs < num_hq:
                if const_expr(_SPLIT):
                    for g in range_constexpr(_PV_GROUPS):
                        for e in range_constexpr(4):
                            d_out = fx.Int32(g * MFMA_N) + k_grp * fx.Int32(4) + fx.Int32(e)
                            buffer_ops.buffer_store(final_pv_sc[g * 4 + e], out_rsrc,
                                                    _po_base + d_out)
                else:
                    for g in range_constexpr(_PV_GROUPS):
                        for e in range_constexpr(4):
                            d_out = fx.Int32(g * MFMA_N) + k_grp * fx.Int32(4) + fx.Int32(e)
                            out_val = _FX_OUT(arith.unwrap(
                                fx.Float32(arith.mulf(final_pv_sc[g * 4 + e],
                                                       arith.unwrap(inv_sum)))))
                            buffer_ops.buffer_store(arith.unwrap(out_val), out_rsrc,
                                                    out_base + d_out)
...
def _make_generic_jit_launcher(
    head_size: int, kv_dtype_str: str, out_dtype_str: str, split_k: int,
) -> Any:  # pyre-ignore[3]
    kernel, _alloc = compile_pa_decode_generic(
        head_size=head_size, kv_dtype_str=kv_dtype_str,
        output_dtype_str=out_dtype_str, split_k=split_k,
    )

    @flyc.jit
    def _launcher(
        out_ptr: fx.Tensor, pm_ptr: fx.Tensor, ps_ptr: fx.Tensor,
        ...
        scale: fx.Float32, split_total: fx.Int32, grid_x: fx.Int32,
    ) -> None:
        ...
        with _ir.InsertionPoint(ctx.gpu_module_body):
            _alloc.finalize()
        kernel(
            out_ptr, pm_ptr, ps_ptr, q_ptr, k_ptr, v_ptr, seq_ptr,
            stride_qb, stride_qg, stride_qh,
            stride_kb, stride_km, stride_kg, stride_kh,
            num_hq, num_g, kv_max, num_hkv, scale, split_total,
...
def pa_decode_generic_launch(
    ...
    out_str = {torch.float16: "f16", torch.bfloat16: "bf16",
               torch.float32: "f32"}[output_dtype]
    ...
    if split_k == 1:
        ...
        run_compiled(launcher, out, dummy, dummy, Q, K, V, seq_positions,
                     ...
                     H_q, G, KV_MAX, H_kv, softmax_scale, split_k, grid_x)
    else:
