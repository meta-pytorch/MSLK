...
def compile_pa_decode_gfx950(
    *,
    ...
    _mfma = rocdl.mfma_f32_16x16x32_bf16 if kv_dtype_str == "bf16" else rocdl.mfma_f32_16x16x32_f16
    ...
    _DC        = _HEAD // 16          # dpasses (8 at D=128)
    ...
    def pa_decode_gfx950_kernel(
        ...
        ratio: fx.Int32, softmax_scale: fx.Float32, split_total: fx.Int32,
    ) -> None:
        ...
        v_lds_f16 = lds_base + fx.Index(_P_BYTES // 2)   # V tile (transpose) after P region
        ...
        for _tile_i, state in range(fx.Index(t_start), fx.Index(t_end),
            ...
            for _r in range_constexpr(_NUM_DMA_V):
                ...
                _v8s.append(buffer_ops.buffer_load(v_rsrc, kv_base + (tile_start + _tok) * stride_km + _col,
                                                   vec_width=8, dtype=_FX_KV))
                _dst   = v_lds_f16 + fx.Index(_dp) * fx.Index(TILE_N * 16) + fx.Index(_tok) * fx.Index(16) + fx.Index(_half) * fx.Index(8)
                _v8dst.append(_dst)
            ...
            for st in range_constexpr(N_SUBTILE):
                ...
                for g in range_constexpr(_QK_GRP):
                    ...
                    k_off  = kv_base + k_tok * stride_km + fx.Int32(g * MFMA_K_QK) + grp * fx.Int32(8)
                    k8 = buffer_ops.buffer_load(k_rsrc, k_off, vec_width=8, dtype=_FX_KV)
            ...
            for e in range_constexpr(4):
                ...
                for st in range_constexpr(N_SUBTILE):
                    s = fx.Float32(vector.extract(qk_st[st], static_position=[e], dynamic_position=[]))
                    s = fx.Float32(arith.mulf(arith.unwrap(s), arith.unwrap(softmax_scale)))
                    # mask out-of-range tokens
                    tok_abs = tile_start + fx.Int32(st * MFMA_N) + tok_lane
                    ok = tok_abs < t_end
                    s = fx.Float32(arith.select(arith.unwrap(ok), arith.unwrap(s), c_neginf))
                    loc = _mxf(loc, s)
                    qk_st[st] = vector.insert(arith.unwrap(s), qk_st[st],
                                              static_position=[e], dynamic_position=[])
                for sh in (1, 2, 4, 8):
                    loc = _mxf(loc, dpp_xor_f32(loc, sh))
                nm = _mxf(rmax[e], loc)
                new_max.append(nm)
                a = _exp2_fast(fx.Float32(arith.mulf(
            ...
            for e in range_constexpr(4):
                ...
                for st in range_constexpr(N_SUBTILE):
                    s = fx.Float32(vector.extract(qk_st[st], static_position=[e], dynamic_position=[]))
                    p = _exp2_fast(fx.Float32(arith.mulf(
                        arith.subf(arith.unwrap(s), arith.unwrap(new_max[e])),
                        arith.constant(LOG2E, type=T.f32))))
                    # masked lanes gave s=-inf -> p=0
                    p = fx.Float32(arith.select(arith.unwrap(new_max[e]) > c_neginf, arith.unwrap(p), c_zero))
                    tile_sum[e] = fx.Float32(arith.addf(arith.unwrap(tile_sum[e]), arith.unwrap(p)))
                    tok = fx.Int32(st * MFMA_N) + tok_lane
                    vector.store(fx.Vector.from_elements([arith.unwrap(p)], dtype=fx.Float32),
                                 p_lds, [fx.Index(head * fx.Int32(TILE_N) + tok)])
            for e in range_constexpr(4):
                for sh in (1, 2, 4, 8):
                    tile_sum[e] = fx.Float32(arith.addf(arith.unwrap(tile_sum[e]),
                                                        arith.unwrap(dpp_xor_f32(tile_sum[e], sh))))
                rsum[e] = fx.Float32(arith.addf(
            ...
            for _r in range_constexpr(_NUM_DMA_V):
                _sp = buffer_ops.create_llvm_ptr(fx.Int64(_v8dst[_r] * fx.Index(2)), address_space=3)
            ...
            for dpass in range_constexpr(_DN):
                for e in range_constexpr(4):
                    acc[dpass * 4 + e] = fx.Float32(arith.mulf(
            ...
            for j in range_constexpr(8):
                pv = fx.Vector.load(T.vec(1, T.f32), p_lds,
                                    [fx.Index(p_head * fx.Int32(TILE_N) + grp * fx.Int32(8) + fx.Int32(j))])[0]
                p_vals.append(arith.truncf(_FX_KV.ir_type, arith.unwrap(fx.Float32(pv))))
            p_frag = zero_v8h
            for j in range_constexpr(8):
                p_frag = vector.insert(p_vals[j], p_frag, static_position=[j], dynamic_position=[])
            ...
            for dpass in range_constexpr(_DN):
                ...
                _GB = fx.Int32(dpass * (TILE_N * 16)) + (grp * fx.Int32(8)) * fx.Int32(16)
                _off_lo = v_lds_f16 + fx.Index(_GB) + (fx.Index(tok_lane)) * fx.Index(4)
                _vlo = rocdl.ds_read_tr16_b64(_v4h, buffer_ops.create_llvm_ptr(
                    fx.Int64(_off_lo * fx.Index(2)), address_space=3)).result
                _off_hi = _off_lo + fx.Index(4 * 16)
                _vhi = rocdl.ds_read_tr16_b64(_v4h, buffer_ops.create_llvm_ptr(
                    fx.Int64(_off_hi * fx.Index(2)), address_space=3)).result
                v_frag = vector.shuffle(_vlo, _vhi, [0, 1, 2, 3, 4, 5, 6, 7])
                c_in = zero_v4
                for e in range_constexpr(4):
                    c_in = vector.insert(arith.unwrap(acc[dpass * 4 + e]), c_in,
                                         static_position=[e], dynamic_position=[])
                c_out = _mfma(T.vec(4, T.f32), [p_frag, v_frag, c_in, 0, 0, 0])
                for e in range_constexpr(4):
                    acc[dpass * 4 + e] = fx.Float32(vector.extract(c_out, static_position=[e], dynamic_position=[]))

            gpu.barrier()  # P_LDS reused next tile

            state_out = ([arith.unwrap(rmax[i]) for i in range(4)]
        ...
        for e in range_constexpr(4):
            ...
            safe_sum = fx.Float32(arith.select(arith.unwrap(f_sum[e]) > c_zero, arith.unwrap(f_sum[e]), c_one))
            inv = rcp_f32(safe_sum)
            if const_expr(_SPLIT):
                _pm_base = (b_idx * (num_g * split_total * num_hq)
                            + g_idx * (split_total * num_hq)
                            + split_idx * num_hq + head_abs)
                _po_base = _pm_base * fx.Int32(_HEAD)
                if (head < ratio) & (head_abs < num_hq):
                    for dpass in range_constexpr(_DN):
                        d = fx.Int32(dpass * MFMA_N) + tok_lane
                        buffer_ops.buffer_store(arith.unwrap(f_acc[dpass * 4 + e]), out_rsrc, _po_base + d)
                    if tok_lane == fx.Int32(0):
                        buffer_ops.buffer_store(arith.unwrap(f_max[e]), pm_rsrc, _pm_base)
                        buffer_ops.buffer_store(arith.unwrap(f_sum[e]), ps_rsrc, _pm_base)
            else:
                ...
                if (head < ratio) & (head_abs < num_hq):
                    for dpass in range_constexpr(_DN):
                        ...
                        buffer_ops.buffer_store(arith.unwrap(out_val), out_rsrc, out_base + d)
...
def _make_gfx950_jit_launcher(head_size, kv_dtype_str, out_dtype_str, split_k):
    kernel, _alloc = compile_pa_decode_gfx950(head_size=head_size, kv_dtype_str=kv_dtype_str,
    ...
    def _launcher(out_ptr, pm_ptr, ps_ptr, q_ptr, k_ptr, v_ptr, seq_ptr,
                  ...
                  num_hq, num_g, kv_max, num_hkv, ratio, scale, split_total, grid_x):
        ...
        kernel(out_ptr, pm_ptr, ps_ptr, q_ptr, k_ptr, v_ptr, seq_ptr,
               stride_qb, stride_qg, stride_qh, stride_kb, stride_km, stride_kg, stride_kh,
               num_hq, num_g, kv_max, num_hkv, ratio, scale, split_total).launch(
                   grid=(grid_x, 1, 1), block=(BLOCK, 1, 1))
    return _launcher


def pa_decode_gfx950_launch(Q, K, V, seq_positions, softmax_scale, split_k=0, output_dtype=None):
    ...
    ok = (H_kv > 0 and H_q % H_kv == 0 and 1 <= ratio <= MFMA_M
    ...
    if split_k == 0: split_k = auto_split_k_hp(B,G,H_q,H_kv,KV_MAX)
    ...
    if split_k == 1:
        ...
        run_compiled(launcher,out,dummy,dummy,Q,K,V,seq_positions,
                     ...
                     H_q,G,KV_MAX,H_kv,ratio,softmax_scale,split_k,n_cta_base)
    else:
        ...
        run_compiled(launcher,po,pm,ps,Q,K,V,seq_positions,
                     sq[0],sq[2],sq[3],sk2[0],sk2[1],sk2[2],sk2[3],
                     H_q,G,KV_MAX,H_kv,ratio,softmax_scale,split_k,n_cta_base*split_k)
