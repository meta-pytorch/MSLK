...
from flydsl.expr import arith, buffer_ops, const_expr, gpu, range_constexpr, rocdl, vector  # pyre-ignore[21]
...
from .utils import exp_f32, maxnumf, rcp_f32, wave_reduce_max_f32, wave_reduce_sum_f32, WARP_SIZE
...
def _compile_reduce(
    head_size: int, max_parts: int, output_dtype_str: str, arch: str,
    ...
    def _kernel(
        ...
        s_po_b:   Int32, s_po_g:  Int32, s_po_part: Int32, s_po_hq: Int32,
        # partial_max/sum strides: [B, G, SK, Hq] — Hq innermost (stride=1)
        s_pm_b:   Int32, s_pm_g:  Int32, s_pm_part: Int32,
        # output strides: [B, G, Hq, D]
        s_o_b:    Int32, s_o_g:   Int32, s_o_hq: Int32,
    ) -> None:
        ...
        if const_expr(_FAST):
            ...
            gsum     = arith.unwrap(wave_reduce_sum_f32(fx.Float32(arith.mulf(w_f32, part_sum))))
            inv_sum  = arith.unwrap(rcp_f32(fx.Float32(
                arith.select(gsum > c_zero, gsum, c_one)
            )))
            ...
            for p in range_constexpr(_MAX_PARTS):
                ...
                vals = buffer_ops.buffer_load(po_rsrc, poff + base_hd, vec_width=_CHUNKS, dtype=T.f32)
                if const_expr(_CHUNKS == 1):
                    accs[0] = arith.addf(accs[0], arith.mulf(vals, wf32))
                else:
                    for c in range_constexpr(_CHUNKS):
                        val = vector.extract(vals, static_position=[c], dynamic_position=[])
                        accs[c] = arith.addf(accs[c], arith.mulf(val, wf32))

            for c in range_constexpr(_CHUNKS):
                out_val = _OUT_FX(arith.unwrap(fx.Float32(accs[c])))
                buffer_ops.buffer_store(arith.unwrap(out_val), out_rsrc, o_base + base_hd + fx.Int32(c))
            ...
            for step in range_constexpr((_MAX_PARTS + WARP_SIZE - 1) // WARP_SIZE):
                ...
                if const_expr(p < _MAX_PARTS):
                    ...
                    vector.store(fx.Vector.from_elements([lm], dtype=fx.Float32),
                                 lm_lds, [fx.Index(arith.constant(p, type=T.i32))])
                    vector.store(fx.Vector.from_elements([ls], dtype=fx.Float32),
                                 ls_lds, [fx.Index(arith.constant(p, type=T.i32))])
            gpu.barrier()

            gmax = c_neginf
            for p in range_constexpr(_MAX_PARTS):
                v = fx.Vector.load(T.vec(1, T.f32), lm_lds, [fx.Index(arith.constant(p, type=T.i32))])[0]
                gmax = arith.maximumf(gmax, arith.unwrap(fx.Float32(v)))

            gsum = c_zero
            accs = [c_zero] * _CHUNKS
            for p in range_constexpr(_MAX_PARTS):
                vm = fx.Vector.load(T.vec(1, T.f32), lm_lds, [fx.Index(arith.constant(p, type=T.i32))])[0]
                vs = fx.Vector.load(T.vec(1, T.f32), ls_lds, [fx.Index(arith.constant(p, type=T.i32))])[0]
                ...
                for c in range_constexpr(_CHUNKS):
                    ...
                    val = buffer_ops.buffer_load(po_rsrc, poff + hd, vec_width=1, dtype=T.f32)
...
def _make_reduce_jit_launcher(
    ...
    def _launcher(
        ...
        s_pm_b:    Int32, s_pm_g:   Int32, s_pm_part: Int32,
        s_o_b:     Int32, s_o_g:    Int32, s_o_hq:    Int32,
        grid_b:    Int32, grid_g:   Int32, grid_hq:   Int32,
    ) -> None:
        ...
        kernel(
            output_ptr, partial_out_ptr, partial_max_ptr, partial_sum_ptr,
            s_po_b, s_po_g, s_po_part, s_po_hq,
            s_pm_b, s_pm_g, s_pm_part,
            s_o_b, s_o_g, s_o_hq,
...
def pa_decode_reduce(
    ...
    dtype_str = {torch.float32: "f32", torch.float16: "f16", torch.bfloat16: "bf16"}[output.dtype]
    arch      = get_rocm_arch()
