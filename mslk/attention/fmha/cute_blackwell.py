...
def _convert_input_format(
    ...
    if is_qkv_mxfp8 and query.ndim == 4:
        ...
        if page_table is None:
            ...
            value = value.contiguous().view(torch.float8_e4m3fn).reshape(B, Mkv_padded, Hkv, K)
        else:
    ...
    ) and query.ndim == 4 and not is_qkv_mxfp8
    if should_fold:
...
class FwOpDecode(AttentionFwOpBase):
    ...
    SUPPORTED_DEVICES: Set[str] = {"cuda"}
    SUPPORTED_DTYPES: Set[torch.dtype] = {torch.bfloat16, torch.float16, torch.float8_e4m3fn}
    SUPPORTED_MAX_K = 128
    SUPPORTED_MIN_K = 64
    SUPPORTED_ATTN_BIAS_TYPES: Iterable[Any] = (
        type(None),
