...
def mxfp4_gemm(
    ...
    On gfx950 / SM100 uses native ``tl.dot_scaled`` MFMA. On gfx942 (no native
    FP4 hardware) falls back to the pre-dequant→fp8 path.
    """
    if not _has_native_mxfp4_mfma():
        return mxfp4_dequant_gemm(XQ, WQ, x_scale, w_scale, output=output)
...
def nvfp4_dequant_gemm(
    ...
    layout produced by triton_quantize_nvfp4). NVFP4 weights normally arrive
    pre-quantized from a checkpoint; to derive plain scales from the swizzled
    quantizer, de-swizzle with ``_from_blocked``.
...
def mxfp4_dequant_gemm(
    ...
    scale is a power of two, so the fp8 conversion is exact.
    """
    if XQ.dtype != torch.uint8:
        XQ = XQ.view(torch.uint8)
    if WQ.dtype != torch.uint8:
