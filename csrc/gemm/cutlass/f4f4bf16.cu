...
// SM103 (B300) FORMAT-AWARE tile selectors, one tree per FP4 format. Each tree
// was generated offline from that format's GB300 autotune sweep over the
// unified Sm100 K=256 + ultra K=768 pool: it returns a K=256 Sm100 tile where
// the wide K=768 ultra tile is inefficient and an ultra tile where it wins, so
// each format gets its own optimum. The 1SM/128-M ultra tiles were pruned (they
// almost never win autotune; dropping all 24 costs at most ~1%); these trees
// reference only the retained 2SM/256-M ultra and Sm100 tiles.
...
Kernel_f4f4bf16 get_kernel_via_tuning(
  ...
  // winners never collide at the same shape and the offline
  // heuristic-generation parser sees plain integer M_N_K keys; the
  // format-specific name also keeps ultra and SM100 kernel names from colliding
  // in one file.
