...
at::Tensor f4f4bf16_ultra_256_128_768_2_1_1(
  ...
  if (global_scale) {
    ...
    return _f4f4bf16<NVFP4, 256, 128, 2, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  } else if (mxfp4_block_size == 16) {
    // MXFP4_16: 1x16 block size with E8M0 scales
    return _f4f4bf16<MXFP4_16, 256, 128, 2, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  } else {
    // MXFP4: standard 1x32 block size (default)
    return _f4f4bf16<MXFP4, 256, 128, 2, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  }
}
