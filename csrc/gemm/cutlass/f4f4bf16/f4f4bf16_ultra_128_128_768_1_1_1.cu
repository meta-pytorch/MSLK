...
at::Tensor f4f4bf16_ultra_128_128_768_1_1_1(
  ...
  if (global_scale) {
    ...
    return _f4f4bf16<NVFP4, 128, 128, 1, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  } else if (mxfp4_block_size == 16) {
    // MXFP4_16: 1x16 block size with E8M0 scales
    return _f4f4bf16<MXFP4_16, 128, 128, 1, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  } else {
    // MXFP4: standard 1x32 block size (default)
    return _f4f4bf16<MXFP4, 128, 128, 1, 1, 1, /*UseUltra=*/true>(
        XQ, WQ, x_scale, w_scale, output, global_scale);
  }
}
