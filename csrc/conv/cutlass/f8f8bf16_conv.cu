...
at::Tensor f8f8bf16_conv(
  ...
  // Check the layouts of inputs.
  bool channels_last = activation.strides()[4] == 1;
  // For NDHWC layout, we assume that filter has shape KTRSC
  // Similarly NCDHW inputs must have KCTRS filters.
  int64_t n, d, h, w, c;
  int64_t k, t, r, s, fc;
  ...
  if (channels_last) {
    TORCH_CHECK(
        filter.strides()[4] == 1,
        "Filter must be KTRSC layout for NDHWC input.");
    n = act_sizes[0];
    d = act_sizes[1];
    h = act_sizes[2];
    w = act_sizes[3];
    ...
    TORCH_CHECK(
        activation.strides()[1] == 1,
        "NCDHW inputs must have channels last underlying memory.");
    // We similarly expect inputs to be in the KCTRS layout with channels last
    // memory.
    TORCH_CHECK(
        filter.strides()[1] == 1,
        "NCDHW inputs must have KCTRS filter layout.");
    n = act_sizes[0];
    c = act_sizes[1];
    d = act_sizes[2];
    h = act_sizes[3];
