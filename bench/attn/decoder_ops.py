...
from mslk.attention.fmha.common import AttentionFwOpBase, Inputs, InputsFp8
...
class DecodeOpBase:
    ...
    OP: type[AttentionFwOpBase] | None = None
    ...
    def _get_op(self) -> type[AttentionFwOpBase]:
        if self.OP is None:
            raise RuntimeError(f"{self.name} does not define an fmha operator")
        return self.OP

    def compute(self, *args) -> torch.Tensor:
        q, k, v, attn_bias = args
        inp = Inputs(q, k, v, attn_bias=attn_bias, scale=float(q.shape[-1] ** -0.5))
        out, _ = self._get_op().apply(inp, False)
        return out
...
class TritonFp8(DecodeOpBase):
    ...
    def compute(self, *args) -> torch.Tensor:
        q, ki, vi, ks, vs, attn_bias, scale = args
        inp = InputsFp8(
            q,
            ki,
            vi,
