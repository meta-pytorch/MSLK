...
@register_op
class StackedNVFP4(QuantizeOpBase):
    def __init__(self) -> None:
        super().__init__()
    ...
    def quantize(self, input: torch.Tensor, *args: Any) -> Any:
        ...
        xq, scale = stacked_nvfp4_quantize(m_sizes, input, x_global_scale)
        return xq, scale, x_global_scale

    def dequantize(self, *args: Any) -> torch.Tensor:
        # Grouped dequant is complex due to per-segment padding; return zeros.
        return torch.zeros(self.input_shape, dtype=torch.bfloat16, device=args[0].device)
